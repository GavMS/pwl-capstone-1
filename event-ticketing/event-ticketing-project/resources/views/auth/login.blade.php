                ...
                <form method="POST" action="{{ route('login') }}" class="space-y-5">
                    ...
                    <div class="flex items-center justify-between px-1">
                        <label for="remember_me" class="inline-flex items-center">
                            <input id="remember_me" type="checkbox" class="rounded border-gray-300 text-[#555555] shadow-sm focus:ring-gray-400" name="remember">
                            <span class="ms-2 text-sm text-[#555555]">{{ __('Remember me') }}</span>
                        </label>

                        @if (Route::has('password.request'))
                            <a class="text-sm font-bold text-[#333333] hover:underline" href="{{ route('password.request') }}">
                                {{ __('Forgot Password?') }}
                            </a>
                        @endif
                    </div>
                    ...
                    <p class="text-center text-sm text-[#555555] mt-6">
                        Don't have an account? <a href="{{ route('register') }}" class="font-bold text-[#333333] hover:underline">Register now</a>
                    </p>
                </form>
            </div>
        </div>
