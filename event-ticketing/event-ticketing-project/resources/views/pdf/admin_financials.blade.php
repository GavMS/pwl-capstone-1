...
<html>
<head>
    <title>Admin Financials Report</title>
    <style>
...
<body>
    <h1>Platform Financials Report</h1>
    <div class="summary">
        <p><strong>Total Gross Revenue:</strong> Rp {{ number_format($totalRevenue, 0, ',', '.') }}</p>
        <p><strong>Total Tickets Sold:</strong> {{ number_format($totalTicketsSold) }}</p>
        <p><strong>Platform Fee (5%):</strong> Rp {{ number_format($platformFee, 0, ',', '.') }}</p>
    </div>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="text-right">Rp {{ number_format($data['platform_fee'], 0, ',', '.') }}</td>
                <td class="text-right">Rp {{ number_format($data['organizer_payout'], 0, ',', '.') }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
