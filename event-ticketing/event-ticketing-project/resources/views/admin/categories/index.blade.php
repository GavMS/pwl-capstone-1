<x-app-layout>
            ...
            <div class="max-w-7xl mx-auto py-6 px-6 lg:px-8 flex flex-col md:flex-row md:justify-between md:items-center gap-4">
                <div>
                    ...
                    <p class="text-[#777777] font-medium text-sm lowercase mt-1">
                        manage your event classification
                    </p>
                </div>
                <div class="flex gap-2">
                    <a href="{{ route('admin.categories.create') }}" class="inline-flex items-center px-6 py-3 bg-[#555555] text-white rounded-2xl font-bold text-sm lowercase hover:bg-black transition-all shadow-sm group">
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="border-b border-gray-50">
                                <th class="px-8 py-6 text-[10px] font-bold text-[#999999] uppercase tracking-widest">category name</th>
                                <th class="px-8 py-6 text-[10px] font-bold text-[#999999] uppercase tracking-widest">slug</th>
                        ...
                        <tbody class="divide-y divide-gray-50">
                            @forelse($categories as $category)
                                <tr class="group hover:bg-[#F9F9F8] transition-colors">
                                    <td class="px-8 py-6">
                                        <div class="font-bold text-[#444444] lowercase text-lg tracking-tight">{{ $category->name }}</div>
                                        @if($category->description)
                                            <div class="text-xs text-[#999999] lowercase mt-0.5 mt-1">{{ Str::limit($category->description, 50) }}</div>
                                        @endif
                                    </td>
                                    <td class="px-8 py-6">
                                        <span class="px-3 py-1 bg-[#F4F4F4] text-[#777777] rounded-lg text-xs font-mono lowercase">
                                            {{ $category->slug }}
                                        </span>
                                    </td>
                                    <td class="px-8 py-6 text-right">
                                        <div class="flex justify-end gap-3 transition-opacity">
                                            <a href="{{ route('admin.categories.edit', $category->id_category) }}" class="w-10 h-10 bg-[#F4F4F4] text-[#555555] rounded-xl flex items-center justify-center hover:bg-[#555555] hover:text-white transition-all shadow-sm">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                            </a>
                                            <form action="{{ route('admin.categories.destroy', $category->id_category) }}" method="POST" id="delete-form-{{ $category->id_category }}">
                                                @csrf
                                                @method('DELETE')
                                                <button type="button" onclick="confirmDelete('{{ $category->id_category }}')" class="w-10 h-10 bg-red-50 text-red-400 rounded-xl flex items-center justify-center hover:bg-red-500 hover:text-white transition-all shadow-sm">
    ...
    <script>
        function confirmDelete(id) {
            Swal.fire({
                title: 'are you sure?',
                text: "this will remove the category. events in this category will become uncategorized.",
                icon: 'warning',
